<template>
  <header v-if="!(activeRouter.match(/about/))" :class="{'fixed': curTop > 40}">
    ...
    <ul class="navbox">
        <li v-for="item in navList" :class="{avitve: liActive === item.url}" :key="item.lable" @click="jumpPage(item.url)">
...
export default {
    ...
    data() {
        return {
            navList: [
                {
                    lable: '首页', icon: 'egg-fried', url: '/'
                },{
                    lable: '芝士', icon: 'book', url: '/note'
                },{
                    lable: '莓用小工具', icon: 'bug', url: '/uselessTool'
                },{
                    lable: '关于我', icon: 'brush', url: '/about'
                }
            ]
        }
    },
